...
const searchCategories = async () => {
    ...
    if (form.category_name.length > 2) {
        delayTimer = setTimeout(async () => {
            let response = await fetch(route('admin.products.categories.search', {search: form.category_name}));
...
const searchBrands = async () => {
    ...
    if (form.brand_name.length > 2) {
        delayTimer = setTimeout(async () => {
            let response = await fetch(route('admin.products.brands.search', {search: form.brand_name}));
